...
{
  // 1. 配置时钟
  _clock_init(I2C_SCL_PORT);
  if (I2C_SDA_PORT != I2C_SCL_PORT) _clock_init(I2C_SDA_PORT);
	
  // 2. 配置 GPIO
	GPIO_InitTypeDef GPIO_InitStructure;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_OD; // 必须配置为**通用**开漏输出 (开漏输出也支持输入, 软件控制引脚电平用通用输入)
  ...
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_OD; // 必须配置为**通用**开漏输出 (开漏输出也支持输入, 软件控制引脚电平用通用输入)
	...
	// 3. 释放总线
	GPIO_SetBits(I2C_SCL_PORT, I2C_SCL_PIN);        // 默认为高电平（释放总线状态）
  GPIO_SetBits(I2C_SDA_PORT, I2C_SDA_PIN);        // 默认为高电平（释放总线状态）
}
...
{
  // 1. 先释放总线
  // 为了同时兼容 起始条件(S) 和 重复起始条件(SR), 以免极端情况下被误判为 终止条件(P)
  // 释放顺序要保证为: 先释放 SDA, 再释放 SCL
